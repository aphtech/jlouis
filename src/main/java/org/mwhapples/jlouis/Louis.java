...
public class Louis {
     ...
     *
     */
    public interface TypeForms {
        short PLAIN_TEXT = 0x0000;
        short ITALIC = 0x0001;
        short UNDERLINE = 0x0002;
        short BOLD = 0x0004;
        short SCRIPT = 0x008;
        short TRANS_NOTE = 0x0010;
        short TRANS_1 = 0X0020;
        short TRANS_2 = 0X0040;
        short TRANS_3 = 0X0080;
        short TRANS_4 = 0X0100;
        short TRANS_5 = 0X0200;
        short COMPUTER_BRAILLE = 0x0400;
        short NO_TRANSLATE = 0x0800;
        short NO_CONTRACT = 0x1000;
    }
     ...
     * </p>
     */
    public interface TranslationModes {
        int NO_CONTRACTIONS = 1;
        int COMPBRL_AT_CURSOR = 2;
        int DOTS_IO = 4;
        int COMP8DOTS = 8;
        int PASS1_ONLY = 16;
        int COMPBRL_LEFT_CURSOR = 32;
        int UC_BRL = 64;
        int NO_UNDEFINED_DOTS = 128;
        int PARTIAL_TRANS = 256;
    }

    private final int outRatio;
    private static LogCallback callback;
    private static int encodingSize;
    private static Properties libConfig;
    private static final Logger logger = LoggerFactory.getLogger(Louis.class);
    /**
    ...
    public TranslationResult translate(String trantab, String inbuf,
        ...
        return translate(trantab, inbuf, null, cursorPos, mode);
    }
    ...
    public TranslationResult translate(String trantab, String inbuf,
        ...
        synchronized (THREAD_SAFE_LOCK) {
            ...
            if (Louis.lou_translate(trantab, wInbuf, new IntByReference(inlen),
                    wOutbuf, poutlen, typeformsCopy, null, outPos, inPos,
                    pcursorPos, mode) == 0) {
                throw new TranslationException("Unable to complete translation");
            }
    ...
    public String backTranslateString(String trantab, String inbuf,
        ...
        synchronized (THREAD_SAFE_LOCK) {
            ...
            if (Louis.lou_backTranslateString(trantab, wInbuf, new IntByReference(
                    inlen), wOutbuf, poutlen, typeformsCopy, null, mode) == 0) {
                throw new TranslationException("Unable to complete translation");
            }
            return wOutbuf.getText(poutlen.getValue());
        }
    ...
    public TranslationResult backTranslate(String trantab, String inbuf,
                                           int cursorPos, int mode) throws TranslationException {
        return backTranslate(trantab, inbuf, null, cursorPos, mode);
    }
    ...
    public static class WideChar implements NativeMapped {
        ...
        public WideChar(int length) {
            ...
            if (Louis.encodingSize == 0)
                return;
            this.length = length;
            buffer = new Memory((long) Louis.encodingSize * length);
        }
        ...
        private void setText(String text) {
            try {
                ...
                buffer = new Memory((long) this.length * Louis.encodingSize);
